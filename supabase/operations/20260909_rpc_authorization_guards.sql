...
DO $$BEGIN IF NOT EXISTS(SELECT FROM pg_proc WHERE oid='public.approve_partner_onboarding(uuid)'::regprocedure AND pg_get_userbyid(proowner)='postgres' AND prosecdef AND md5(replace(prosrc,chr(13),'')) IN ('3f4722424b7e4199aff4bd509aefc9be','f6dcf3a37c05950d73c2951cfd8b6b3f')) THEN RAISE EXCEPTION 'RPC_GUARD_REVIEW_REQUIRED: approve_partner_onboarding'; END IF; END$$;
...
END;
$function$;
DO $$BEGIN IF NOT EXISTS(SELECT FROM pg_proc WHERE oid='public.delete_business_safely(uuid,text)'::regprocedure AND pg_get_userbyid(proowner)='postgres' AND prosecdef AND md5(replace(prosrc,chr(13),'')) IN ('3aaf5cf5e95aa1c1ff5ba3cd512b50ca','1af89a3fbcfc749921fc7b8432b27284')) THEN RAISE EXCEPTION 'RPC_GUARD_REVIEW_REQUIRED: delete_business_safely'; END IF; END$$;
...
SELECT true AS rpc_guards_updated;
COMMIT;
