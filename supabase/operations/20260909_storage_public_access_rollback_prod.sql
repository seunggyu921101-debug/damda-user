...
CREATE POLICY "Authenticated users can delete" ON storage.objects AS PERMISSIVE FOR DELETE TO "authenticated" USING ((bucket_id = 'public'::text));
CREATE POLICY "Authenticated users can update" ON storage.objects AS PERMISSIVE FOR UPDATE TO "authenticated" USING ((bucket_id = 'public'::text));
CREATE POLICY "Authenticated users can upload" ON storage.objects AS PERMISSIVE FOR INSERT TO "authenticated" WITH CHECK ((bucket_id = 'public'::text));
...
CREATE POLICY "Public Access" ON storage.objects AS PERMISSIVE FOR SELECT TO "public" USING ((bucket_id = 'public'::text));
COMMIT;
