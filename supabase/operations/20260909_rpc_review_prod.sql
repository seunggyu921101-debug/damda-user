...
DO $$BEGIN IF NOT EXISTS(SELECT FROM pg_proc WHERE oid='public.get_public_popular_businesses(integer)'::regprocedure AND pg_get_userbyid(proowner)='postgres' AND prosecdef AND md5(replace(prosrc,chr(13),'')) IN ('36400bac42772be2bc4290f4078782cc','36400bac42772be2bc4290f4078782cc')) THEN RAISE EXCEPTION 'RPC_REVIEW_REQUIRED: get_public_popular_businesses'; END IF; END$$;
...
AS $function$
  SELECT EXISTS (
    SELECT 1
    FROM public.product_preview_tokens ppt JOIN public.products p ON p.id=ppt.product_id JOIN public.business_owners bo ON bo.id=p.business_owner_id
    WHERE ppt.business_owner_id=p.business_owner_id AND bo.status='active' AND ppt.product_id = p_product_id
...
AS $function$
  SELECT (SELECT jsonb_object_agg(k,v) FROM jsonb_each(to_jsonb(p)) AS projected(k,v) WHERE k=ANY(ARRAY['id','business_owner_id','category_id','name','summary','description','thumbnail','original_price','sale_price','min_participants','max_participants','duration_minutes','address','latitude','longitude','region','available_time_slots','is_visible','is_sold_out','view_count','created_at','updated_at','address_detail','business_id','minimum_age','recommended_age_min','recommended_age_max','booking_start_date','booking_end_date','booking_cutoff_hours','allow_same_day_booking','inclusions','exclusions','materials','usage_method','product_precautions','reservation_notice','refund_notice','other_notice','display_order','experience_environment','operates_in_rain','rain_alternative','bus_accessible','bus_parking_available','dropoff_space_available','meal_available','lunchbox_allowed','restroom_info','child_restroom_available','teacher_supplies','child_supplies','provided_supplies','accessibility_info','meeting_point','field_contact','teacher_notes','clothing_guidance','meal_guidance','transportation_guidance','guardian_notes','facility_services'])) || jsonb_build_object(
    ...
    END,
    'images', coalesce((
      SELECT jsonb_agg((SELECT jsonb_object_agg(k,v) FROM jsonb_each(to_jsonb(pi)) AS projected(k,v) WHERE k=ANY(ARRAY['id','product_id','image_url','sort_order','created_at'])) ORDER BY pi.sort_order)
      FROM public.product_images pi
      WHERE pi.product_id = p.id
    ), '[]'::jsonb),
    'options', coalesce((
      SELECT jsonb_agg((SELECT jsonb_object_agg(k,v) FROM jsonb_each(to_jsonb(po)) AS projected(k,v) WHERE k=ANY(ARRAY['id','product_id','name','price','is_required','sort_order','created_at'])) ORDER BY po.sort_order)
      FROM public.product_options po
      WHERE po.product_id = p.id
    ), '[]'::jsonb),
    'unavailable_dates', coalesce((
      SELECT jsonb_agg((SELECT jsonb_object_agg(k,v) FROM jsonb_each(to_jsonb(pud)) AS projected(k,v) WHERE k=ANY(ARRAY['id','product_id','unavailable_date','is_recurring','day_of_week','created_at'])) ORDER BY pud.unavailable_date)
...
AS $function$
  ...
  SELECT jsonb_build_object(
    'business', (SELECT jsonb_object_agg(k,v) FROM jsonb_each(to_jsonb(b)) AS projected(k,v) WHERE k=ANY(ARRAY['id','business_owner_id','name','address','address_detail','zipcode','latitude','longitude','logo_url','introduction','status','is_primary','created_at','updated_at','summary','parking_available','parking_notice','facilities','common_guide','common_precautions','region','is_visible','category_id'])) || jsonb_build_object(
      'place_profile', (
